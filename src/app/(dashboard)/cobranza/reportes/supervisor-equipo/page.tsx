...
export default function Page() {
  ...
  const columns = useMemo<ColumnDef<ReporteSupervisorEquipoItem>[]>(
    () => [
      ...
      { accessorKey: 'efectividadPct', header: 'Efectividad %', cell: ({ row }) => `${row.original.efectividadPct}%` },
      { accessorKey: 'montoRecuperado', header: 'Recuperado', cell: ({ row }) => formatearMoneda(row.original.montoRecuperado) },
      { accessorKey: 'brechaVsPromedioRecuperado', header: 'Brecha recup.', cell: ({ row }) => formatearMoneda(row.original.brechaVsPromedioRecuperado) },
      { accessorKey: 'brechaVsPromedioEfectividad', header: 'Brecha efect.', cell: ({ row }) => `${row.original.brechaVsPromedioEfectividad}%` },
    ],
    [],
  );
  ...
  const metrics = useMemo<DashboardMetric[]>(() => {
    ...
    const r = reporte;
    return [
      { label: 'Cobradores', value: String(r.totalCobradores) },
      { label: 'Recuperado', value: formatearMoneda(r.totalRecuperado), tone: 'primary' },
      { label: 'Promedio recup.', value: formatearMoneda(r.promedioRecuperado) },
      { label: 'Promedio efect.', value: `${r.promedioEfectividad}%` },
    ];
  }, [reporte]);

  return (
    <div className="space-y-4">
      <PageHeader title="Supervisor vs equipo" description="Ranking del equipo con brechas vs promedio." />
      <div className="space-y-3 rounded-lg border border-stroke bg-white p-4 dark:border-dark-3 dark:bg-gray-dark">
        <div className="flex flex-wrap items-end gap-3">
          ...
          />
          <div>
            <label htmlFor="periodo-supervisor-equipo" className="mb-1 block text-sm font-medium">Periodo</label>
            <input
              id="periodo-supervisor-equipo"
              type="month"
              value={periodo}
