...
import { TRAMOS_MORA } from '@/lib/cobranza/tramos-mora';
...
export default function AsignacionCarteraPage() {
  ...
  const [gestoresSeleccionados, setGestoresSeleccionados] = useState<Set<number>>(
    new Set(),
  );
  ...
  const { data: usuariosData, isLoading: gestoresLoading, error: gestoresError } =
    useGraphQLQuery<{
  ...
  const simularMutation = useGraphQLMutation(SIMULAR_ASIGNACION_CARTERA, {
    onSuccess: (data: unknown) => {
      const result = data as { simularAsignacionCartera: typeof simulacion };
  ...
  const ejecutarMutation = useGraphQLMutation(EJECUTAR_ASIGNACION_CARTERA, {
    // Asignaciones masivas (cientos/miles de préstamos) pueden superar 30s.
  ...
  const construirFiltros = () => {
    const tramo = TRAMOS_MORA.find(
      (t) => `${t.tramoMoraMin}:${t.tramoMoraMax ?? 'null'}` === tramoKey,
    );
    return {
      idmandante: idmandante as number,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Distribuya préstamos entre cobradores con simulación previa"
      />
        ...
        <div>
          ...
          >
            ...
            {TRAMOS_MORA.map((t) => (
              <option
                key={t.tramo}
                value={`${t.tramoMoraMin}:${t.tramoMoraMax ?? 'null'}`}
              >
                {t.tramo}
              </option>
            ))}
      ...
      {simulacion && (
        <div className="rounded-lg border border-stroke p-4 dark:border-dark-3">
          ...
          <div className="mt-4 flex gap-2">
            ...
            >
              {ejecutarMutation.isPending ? 'Asignando...' : 'Confirmar asignación'}
            </Button>
