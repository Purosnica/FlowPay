...
export const CreatePrestamoInputSchema = z.object({
  ...
  estado: z.nativeEnum(EstadoPrestamoEnum).optional(),
  montoSolicitado: z.number().nonnegative({ message: "El monto solicitado no puede ser negativo" }),
  montoAprobado: z.number().nonnegative({ message: "El monto aprobado no puede ser negativo" }).optional(),
  montoDesembolsado: z.number().nonnegative({ message: "El monto desembolsado no puede ser negativo" }).optional(),
  comisionTercerizado: z.number().nonnegative({ message: "La comisión no puede ser negativa" }).optional(),
  tasaInteresAnual: z.number().nonnegative({ message: "La tasa de interés no puede ser negativa" }).optional(),
  plazoMeses: z.number().int().positive({ message: "El plazo debe ser mayor a 0" }).optional(),
...
export const CreatePagoInputSchema = z.object({
  ...
  fechaPago: dateInput,
  referencia: z.string().optional(),
  montoCapital: z.number().nonnegative({ message: "El monto de capital no puede ser negativo" }).default(0),
  montoInteres: z.number().nonnegative({ message: "El monto de interés no puede ser negativo" }).default(0),
  montoMora: z.number().nonnegative({ message: "El monto de mora no puede ser negativo" }).default(0),
  montoTotal: z.number().nonnegative({ message: "El monto total no puede ser negativo" }).optional(),
